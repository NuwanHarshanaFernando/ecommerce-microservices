...
import {Search, User2Icon as ProfileIcon, HeartIcon, ShoppingCartIcon as CartIcon} from 'lucide-react'
...
const HeaderBottom = () => {
  ...
  return (
    <div className={`w-full transition-all duration-300 ${isSticky ? "fixed top-0 left-0 z-[100] bg-white shadow-lg" : "relative"}`}> 
        <div className={`w-[80%] relative m-auto flex items-center justify justify-between ${isSticky ? 'pt-3' : 'py-0'}`}>
            {/* All Dropdowns */}
            ...
            <div className='flex items-center'>
                {navItems.map((i:NavItemsTypes, index:number)=>(
                    <Link className='px-5 text-lg font-medium' href={i.href} key={index}>{i.title}</Link>
                ))}
            </div>
